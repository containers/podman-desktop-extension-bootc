  ...
  <div slot="content" class="p-5 min-w-full h-fit">
    <div class="bg-charcoal-900 pt-5 space-y-6 px-8 sm:pb-6 xl:pb-8 rounded-lg">
      <div class="pb-4">
        ...
        {:else if bootcAvailableImages.length === 0}
      ...
      <div>
        <label for="path" class="block mb-2 text-sm font-bold text-gray-400">Build Context Directory</label>
      ...
      <div class="p-2 space-y-7 h-fit">
        <div class="mb-2">
          ...
          <div class="flex items-center mb-3">
            <label for="raw" class="ml-1 flex items-center cursor-pointer">
              <input bind:group="{buildType}" type="radio" id="raw" name="format" value="raw" class="sr-only peer" />
              <div
          ...
          <div class="flex items-center mb-3">
            <label for="iso" class="ml-1 flex items-center cursor-pointer">
              <input bind:group="{buildType}" type="radio" id="iso" name="format" value="iso" class="sr-only peer" />
              <div
                class="w-4 h-4 rounded-full border-2 border-gray-400 mr-2 peer-checked:border-purple-500 peer-checked:bg-purple-500">
              </div>
          ...
          <div class="flex items-center mb-3">
            <label for="ami" class="ml-1 flex items-center cursor-pointer">
              <input bind:group="{buildType}" type="radio" id="ami" name="format" value="ami" class="sr-only peer" />
              <div
                class="w-4 h-4 rounded-full border-2 border-gray-400 mr-2 peer-checked:border-purple-500 peer-checked:bg-purple-500">
              </div>
        ...
        <div class="mb-2">
          ...
          <ul class="grid grid-cols-2 gap-x-2 max-w-md">
            <li>
              <input bind:group="{buildArch}" type="radio" id="amd64" name="arch" value="amd64" class="sr-only peer" />
              <label
                for="amd64"
            ...
            <li>
              <input bind:group="{buildArch}" type="radio" id="arm64" name="arch" value="arm64" class="sr-only peer" />
              <label
                for="arm64"
                class="h-full flex items-center p-5 cursor-pointer rounded-lg bg-zinc-700 border border-transparent hover:border-violet-500 focus:outline-none peer-checked:ring-2 peer-checked:ring-violet-500 peer-checked:border-transparent">
                <i class="fab fa-linux fa-2x"></i>
        ...
        {#if buildInProgress}
          <Button class="w-full" disabled="{true}"
            >Build in progress. You can close this and view the progress on the Containers tab. You will be notified on
            a completed build.</Button>
        {:else}
